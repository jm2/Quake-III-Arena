...
| Connectionless query and rcon amplification (CVE-2010-5077) | [`4056c903`](https://github.com/ioquake/ioq3/commit/4056c9035825b2c6ffef3378b4af78e7e70bb743), [`a3c7003d`](https://github.com/ioquake/ioq3/commit/a3c7003d9cdde316ef1dc4b2c76428f295b93491), [`7b154150`](https://github.com/ioquake/ioq3/commit/7b1541504216ce09ef2c7987eb5a7117a2125a5a); [#38](https://github.com/jm2/Quake-III-Arena/issues/38) | One global 10-a-second count covered only `getstatus` and rcon; `getinfo` and `getchallenge` were unlimited, and a global 500 msec gate let bad rcon lock out the admin | Locally ported leaky buckets: each source gets 10 `getstatus`, `getinfo`, `getchallenge` or rcon requests at once and one more a second; the queries share 10 at once and one more every 100 msec, and bad rcon passwords 10 at once and one more a second. Lookups are bounded. A correct rcon password is never locked out: when its address has used up its quota (a flood may spoof that address) or no bucket is free, it falls back on an allowance of 10 at once and one more a second that only correct passwords can use. As with ioquake3, when many players refresh the LAN list at once (32 players send 64 `getinfo`), some miss the server on the first pass and see it on the next refresh |
