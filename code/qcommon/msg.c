...
void MSG_WriteBits( msg_t *msg, int value, int bits ) {
	...
	if (msg->oob) {
		...
		if (bits==8) {
			...
			short temp = LittleShort(value);
			Com_Memcpy(&msg->data[msg->cursize], &temp, sizeof(temp));
			msg->cursize += 2;
			msg->bit += 16;
		} else if (bits==32) {
			int temp = LittleLong(value);
			Com_Memcpy(&msg->data[msg->cursize], &temp, sizeof(temp));
			msg->cursize += 4;
			msg->bit += 32;
		} else {
...
int MSG_ReadBits( msg_t *msg, int bits ) {
	...
	if (msg->oob) {
		...
		if (bits==8) {
			...
			short temp;
			Com_Memcpy(&temp, &msg->data[msg->readcount], sizeof(temp));
			value = LittleShort(temp);
			msg->readcount += 2;
			msg->bit += 16;
		} else if (bits==32) {
			Com_Memcpy(&value, &msg->data[msg->readcount], sizeof(value));
	...
	if ( sgn && originalBits > 0 && originalBits < 32 ) {
		if ( value & ( 1 << ( originalBits - 1 ) ) ) {
			value |= -1 ^ ( ( 1 << originalBits ) - 1 );
		}
	}
...
void MSG_ReadDeltaEntity( msg_t *msg, entityState_t *from, entityState_t *to, 
	...
	// CVE-2018-12100 family: lc comes straight off the wire; if it exceeds
	// numFields the loop below walks past entityStateFields and reads /
	// writes garbage memory. Treat it as a fatal protocol error.
...
void MSG_ReadDeltaPlayerstate (msg_t *msg, playerState_t *from, playerState_t *to ) {
	...
	*to = *from;
	...
	// CVE-2018-12100 family: see MSG_ReadDeltaEntity for context.
	if ( lc > numFields || lc < 0 ) {
