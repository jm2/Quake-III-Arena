...
Q3_TEST_BINARY="$(mktemp "${TMPDIR:-/var/tmp}/q3-bot-action-setup.XXXXXX")"
...
for Q3_TEST_MODE in normal fast debug debug-fast manager manager-fast;do
    ...
    "${CC:-cc}" -DBOTLIB -std=gnu99 -fgnu89-inline -fno-omit-frame-pointer -ffunction-sections -fdata-sections \
        ...
        "-DQ3_AAS_SETUP_SOURCE=\"$Q3_TEST_ROOT/code/botlib/be_aas_main.c\"" \
        "$Q3_TEST_ROOT/tests/bot_action_setup_regression.c" "$Q3_TEST_ROOT/code/botlib/l_libvar.c" "$Q3_TEST_ROOT/code/botlib/l_memory.c" "$Q3_TEST_ROOT/code/botlib/be_interface.c" "$Q3_TEST_ROOT/code/botlib/be_aas_entity.c" "$Q3_TEST_ROOT/code/botlib/be_ea.c" "$Q3_TEST_ROOT/code/game/q_shared.c" \
