...
Q3_TEST_BINARY="$(mktemp "${TMPDIR:-/var/tmp}/q3-vm-memory-traps.XXXXXX")"
trap 'rm -f -- "$Q3_TEST_BINARY"' EXIT

"${CC:-cc}" \
    -std=gnu99 -fno-omit-frame-pointer -ffunction-sections -fdata-sections \
    -fsanitize=address,undefined \
    "$Q3_TEST_ROOT/tests/vm_memory_trap_regression.c" \
...
# separately asserts file ownership and releases all simulated hunk storage.
