 ...
 * shows on, as the renderer reads a nonzero value. */
...
int main( int argc, char **argv ) {
	static const char *names[] = { "r_mode", "r_fullscreen", "r_allowExtensions" };
	menulist_s *items[3];
	menulist_s *item;
	char *end;
	long v;
	...
	if ( argc != 3 ) {
		fprintf( stderr, "usage: %s r_mode|r_fullscreen|r_allowExtensions <value>\n", argv[0] );
		return 2;
	}
	cvar = argv[1];
	value = argv[2];
	...
	for ( i = 0; i < 3 && strcmp( cvar, names[i] ); i++ ) {
	}
	Check( i < 3, "r_mode, r_fullscreen or r_allowExtensions" );
