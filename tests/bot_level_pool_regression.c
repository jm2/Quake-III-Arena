...
/* Smallest power-of-two count whose pool cost exceeds INT_MAX: 2^25 with the
   LP64 levelitem_t, 2^26 with the smaller ILP32 one (the PPC target). */
static unsigned int CostBits(void){float count=1;unsigned int bits;while(count<=(float)(INT_MAX/sizeof(levelitem_t)))count*=2;memcpy(&bits,&count,sizeof(bits));return bits;}
