...
Q3_TEST_DIR="$(mktemp -d -p "${TMPDIR:-/var/tmp}" q3-message-huffman.XXXXXX)"
...
for Q3_TEST_MODE in normal fast; do
    ...
    "${CC:-cc}" -std=gnu99 -fno-omit-frame-pointer -ffunction-sections -fdata-sections \
        ...
        "$Q3_TEST_ROOT/tests/message_huffman_regression.c" \
        "$Q3_TEST_ROOT/code/game/q_shared.c" -Wl,--gc-sections -lm \
        -o "$Q3_TEST_DIR/message-huffman-tests"
    for Q3_CASE in {0..25}; do
        ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 \
        UBSAN_OPTIONS=halt_on_error=1:print_stacktrace=1 \
            "$Q3_TEST_DIR/message-huffman-tests" "$Q3_CASE"
    done
done
