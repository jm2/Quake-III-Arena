...
Q3_TEST_DIR="$(mktemp -d "${TMPDIR:-/var/tmp}/q3-game-vote.XXXXXX")"
trap 'rm -rf -- "$Q3_TEST_DIR"' EXIT

# Link the real native g_cmds.c (ClientCommand, Cmd_Vote_f, Cmd_TeamVote_f)
# as the monolithic static build does and cast "vote <arg>" and
# "teamvote <arg>" ballots through ClientCommand. The test stubs the engine
# traps and the game symbols the file's other commands reference. Built for
# base Quake III and Team Arena (MISSIONPACK), which compile the same
# g_cmds.c.
for Q3_TEST_VARIANT in base missionpack; do
    ...
    "${CC:-cc}" \
        ...
        "$Q3_TEST_ROOT/tests/game_vote_regression.c" \
