...
static int engineReady,queries;
...
int BotInitLibrary(void){engineReady=0;return setupError;}
...
int trap_BotLibLoadMap(const char *name){Check(!strcmp(name,"native"),"native map name");loads++;engineReady=loadError==BLERR_NOERROR;return loadError;}
int trap_BotLibVarGet(char *name,char *value,int size){Check(!strcmp(name,"botlib_mapready")&&size==2,"engine retained map status query through unchanged syscall");queries++;value[0]=engineReady<0?0:engineReady?'1':'0';value[1]=0;return BLERR_NOERROR;}
...
static void Begin(int occupied){memset(botstates,0,sizeof(botstates));memset(&priorBot,0,sizeof(priorBot));priorBot.inuse=qtrue;priorBot.setupcount=9;priorBot.client=7;if(occupied)botstates[7]=&priorBot;botmapready=occupied;engineReady=occupied;queries=0;loadError=setupError=loads=resets=dmSetups=initBots=remaps=warnings=0;enabled=1;g_gametype.integer=GT_FFA;g_log.string[0]=0;g_maxclients.integer=2;}
...
static void FreshRestart(int ready){Begin(0);engineReady=ready;Check(BotAISetup(1)&&!botmapready,"fresh game VM restart starts with cleared module readiness");Check(BotAILoadMap(1)==ready&&queries==1&&!loads&&!resets&&dmSetups==ready&&botmapready==ready,"fresh VM restores only complete engine map status without reload");Check(BotAIStartFrame(100)==(ready?73:0)&&BotAISetupClient(7,NULL,0)==(ready?74:0),"fresh VM restart gates frame/client from engine status");Begin(0);engineReady=ready;G_InitGame(300,7,1);Check(botmapready==ready&&queries==1&&!loads&&dmSetups==ready&&initBots==ready&&remaps==1&&warnings==!ready,"fresh VM game initializer restores native retained map or continues human play after failure");}
static void LegacyRestart(void){Begin(0);engineReady=-1;Check(BotAISetup(1)&&!botmapready&&BotAILoadMap(1)&&botmapready&&queries==1&&!loads&&!resets&&dmSetups==1,"legacy engine empty unknown variable preserves normal fresh-VM restart");Check(BotAIStartFrame(100)==73&&BotAISetupClient(7,NULL,0)==74,"legacy fresh-VM restart reaches native frame/client imports");Begin(0);engineReady=-1;G_InitGame(300,7,1);Check(botmapready&&queries==1&&!loads&&dmSetups==1&&initBots==1&&remaps==1&&!warnings,"legacy game initializer preserves native restarted bot initialization");}
int main(int argc,char **argv){int occupied;if(argc>1){if(atoi(argv[1])==0)MapFailure(1,BLERR_LIBRARYNOTSETUP);else if(atoi(argv[1])==1)InitFailure(0);else if(atoi(argv[1])==3)FreshRestart(1);else if(atoi(argv[1])==4)LegacyRestart();else{MapGolden();InitGolden();}return 0;}for(occupied=0;occupied<2;occupied++){MapFailure(occupied,BLERR_LIBRARYNOTSETUP);MapFailure(occupied,BLERR_CANNOTOPENAASFILE);}InitFailure(0);InitFailure(1);MapGolden();InitGolden();FreshRestart(0);FreshRestart(1);LegacyRestart();puts("Actual game map/setup errors disable bots before reset/frame/client imports; native retry/restart/human setup pass (issues #47/#48)");return 0;}
