...
int trap_BotLibVarGet(char *name,char *value,int size){Check(!strcmp(name,"botlib_mapready")&&size==2,"engine retained map status query through unchanged syscall");queries++;value[0]=engineReady?'1':'0';value[1]=0;return BLERR_NOERROR;}
...
int main(int argc,char **argv){int occupied;if(argc>1){if(atoi(argv[1])==0)MapFailure(1,BLERR_LIBRARYNOTSETUP);else if(atoi(argv[1])==1)InitFailure(0);else if(atoi(argv[1])==3)FreshRestart(1);else{MapGolden();InitGolden();}return 0;}for(occupied=0;occupied<2;occupied++){MapFailure(occupied,BLERR_LIBRARYNOTSETUP);MapFailure(occupied,BLERR_CANNOTOPENAASFILE);}InitFailure(0);InitFailure(1);MapGolden();InitGolden();FreshRestart(0);FreshRestart(1);puts("Actual game map/setup errors disable bots before reset/frame/client imports; native retry/restart/human setup pass (issues #47/#48)");return 0;}
