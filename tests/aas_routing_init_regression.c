...
static int Open(const char *name,fileHandle_t *file,fsMode_t mode) {Check(!strcmp(name,"maps/synthetic.rcd")&&mode==FS_READ,"bounded native cache lookup");cacheReads++;*file=0;return -1;}
...
int AAS_ContinueInitReachability(float time) {(void)time;return 0;}
...
static void Reset(int empty) {
    ...
    botimport.Print=Print;botimport.FS_FOpenFile=Open;
}
...
int main(int argc,char **argv) {
    int i;if(argc>1){int proof=atoi(argv[1]);if(proof==0)Nullable(1);else if(proof==1){Reset(0);aasworld.numareas=aasworld.numareasettings=INT_MAX;AAS_ContinueInit(7);}else MatrixOverflow();}
    else {Success();for(i=1;i<=10;i++)Nullable(i);EmptyDummy();ShapeCosts();puts("Native AAS routing initialization costs, nullable stages and publication passed (issue #47)");}
    return 0;
}
