#!/usr/bin/env bash
# Issue #426: the real Q_rand/Q_random/Q_crandom over many seeds must match the
# wrapped uint32_t LCG bit for bit without a signed overflow (no UBSan recover).
set -euo pipefail
export TMPDIR="${TMPDIR:-/var/tmp}"
Q3_TEST_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
Q3_TEST_BINARY="$(mktemp "${TMPDIR:-/var/tmp}/q3-q-rand.XXXXXX")"
...
"${CC:-cc}" -std=gnu99 -fno-omit-frame-pointer -ffunction-sections -fdata-sections \
    ...
    "$Q3_TEST_ROOT/tests/q_rand_regression.c" "$Q3_TEST_ROOT/code/game/q_math.c" \
